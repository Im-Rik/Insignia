...
const AutomatedEditor = forwardRef(({ videoRef, isRecording, onEditorStateChange }, ref) => {
    ...
    const [isProcessing, setIsProcessing] = useState(false); // Disables buttons during any background task
    const [isSegmenting, setIsSegmenting] = useState(false); // Shows the blocking loader overlay
    ...
    const processVideoOnServer = async (videoBlob) => {
        if (!videoBlob) {
            setStatusMessage("Error: No video data available.");
            return;
        }
        ...
        setLoopingSegment(null);
        setSegments([]);
        setStatusMessage("Analyzing and segmenting...");
        try {
    ...
    const handleSendSegments = async () => {
        ...
        setIsProcessing(true); // Only disable buttons, don't show overlay
        setLoopingSegment(null);
        ...
        try {
            ...
            es.onmessage = (event) => {
                ...
                // Add a completion check from server
    ...
    const playSegmentInLoop = (segment) => {
        if (!editorVideoRef.current) return;
        setLoopingSegment(segment);
        editorVideoRef.current.currentTime = segment.start;
                    ...
                    <div className="flex-grow overflow-y-auto pr-2 space-y-2">
                        {segments.length > 0 ? segments.map((seg, i) => (
                            ...
                            <div className="text-gray-500 text-center mt-8 px-4">
                                {isProcessing ? 'Analyzing...' : 'No segments found yet. Please use Auto-Detect.'}
                            </div>
                        )}
                    </div>
                </div>
