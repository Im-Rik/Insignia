...
// --- A more visually appealing "pending" indicator ---
...
// A non-blocking loader for the side panel
...
const AutomatedEditor = forwardRef(({ videoRef, isRecording, onEditorStateChange }, ref) => {
    ...
    const processVideoOnServer = async (videoBlob) => {
        ...
        setIsProcessing(true); setIsSegmenting(true); setLoopingSegment(null); setSegments([]);
        setStatusMessage("Uploading and analyzing for segments...");
        try {
            const formData = new FormData();
            // --- MODIFICATION #1: Use the blob's actual name for the upload ---
            // This works for both uploaded files (e.g., "my_video.mp4") and recorded files ("recorded_video.webm")
        ...
        try {
            ...
            es.onmessage = (event) => {
                ...
                if (data.status === 'complete') {
                    setStatusMessage("✅ All segments classified!");
                    setIsProcessing(false); es.close(); return;
                }
                if (data.error) { console.error("Received error from server:", data);
                } else {
                    receivedData = true;
            ...
            es.onerror = (err) => {
                ...
                setStatusMessage(receivedData ? "Connection to server lost." : "Could not connect to classification server.");
    ...
    const handleResetEditor = () => { if (videoSrc && videoSrc.startsWith('blob:')) URL.revokeObjectURL(videoSrc); setVideoSrc(null); setOriginalBlob(null); setIsPlaying(false); setDuration(0); setCurrentTime(0); setSegments([]); setIsProcessing(false); setIsSegmenting(false); setLoopingSegment(null); onEditorStateChange(false); setStatusMessage('Record or upload a video to begin.'); setEditorMode('welcome'); if (eventSourceRef.current) eventSourceRef.current.close(); };
    ...
    useEffect(() => {
        ...
        if (isRecording && stream) {
            ...
            mediaRecorderRef.current.onstop = () => {
                ...
                // --- MODIFICATION #3: Create a File object with a name and type ---
                // This ensures the recorded blob has a ".name" property, just like an uploaded file.
                const recordedFile = new File([blob], "recorded_video.webm", {
                    type: "video/webm",
                    lastModified: Date.now(),
                });

                const url = URL.createObjectURL(recordedFile);
                setOriginalBlob(recordedFile); // Store the new File object
                setVideoSrc(url);
    ...
    const handleFileChange = (event) => {
        ...
        if (file) {
            ...
            setOriginalBlob(file); // An uploaded file is already a `File` object with a .name
            onEditorStateChange(true);
            setEditorMode('editing');
            ...
            <div className="flex-grow flex flex-col md:flex-row gap-4 min-h-0">
                ...
                <div className="w-full md:w-1/3 bg-gray-800 rounded-lg p-3 flex flex-col">
                    ...
                    <div className="flex-grow min-h-0">
                        {isSegmenting ? (
                            <AnalysisLoader status={statusMessage} />
                        ) : (
                            <>
                                <p className="text-sm text-gray-400 mb-3 flex-shrink-0 h-10">{statusMessage}</p>
                                <div className="h-[calc(100%-3.5rem)] overflow-y-auto pr-2 space-y-2">
                                    {segments.length > 0 ? segments.map((seg, i) => (
                                        ...
                                        <div className="text-gray-500 text-center mt-8 px-4">
                                            No segments found. Use 'Auto-Detect' to find them.
                                        </div>
                                    )}
                                </div>
                            </>
            ...
            <div className="flex-shrink-0 p-3 bg-gray-800/90 rounded-lg flex items-center gap-4 flex-wrap">
                 ...
                 <div ref={timelineContainerRef} onMouseDown={handleScrubStart} onTouchStart={handleScrubStart} className="flex-grow h-4 flex items-center group relative cursor-pointer pr-2">
                     <div className="w-full h-2 bg-gray-600 rounded-full relative">
                         {segments.map(seg => (
                             <div key={`timeline-${seg.id}`} className={`absolute h-full opacity-75 rounded-full ${loopingSegment?.id === seg.id ? 'bg-cyan-400' : 'bg-green-500'}`} style={{ left: `${duration > 0 ? (seg.start / duration) * 100 : 0}%`, width: `${duration > 0 ? ((seg.end - seg.start) / duration) * 100 : 0}%` }}></div>
                         ))}
                         <div className="absolute h-full" style={{ left: `${duration > 0 ? (currentTime / duration) * 100 : 0}%`, pointerEvents: 'none' }}>
                             <div className="w-4 h-4 bg-white rounded-full -translate-x-1/2 -translate-y-1/2 top-1/2 absolute"></div>
                         </div>
                     </div>
                 </div>
                 <div className="font-mono text-sm text-gray-500">{formatTime(duration)}</div>
                 <div className="w-px h-6 bg-gray-700"></div>
                 <button onClick={() => processVideoOnServer(originalBlob)} disabled={!originalBlob || isProcessing} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-semibold px-4 py-1.5 rounded-md">
                     <AutoIcon /> {isSegmenting ? 'Analyzing...' : 'Auto-Detect'}
                 </button>
                 <button onClick={handleSendSegments} disabled={isProcessing || segments.length === 0} className="flex items-center gap-2 bg-green-600 hover:bg-green-500 disabled:opacity-50 text-white font-semibold px-4 py-1.5 rounded-md">
                     <SendIcon /> Send to Classifier
                 </button>
             </div>
        </div>
    );
        ...
        <div className="w-full h-full flex flex-col items-center justify-center p-4 gap-8 bg-gray-900 text-white">
            ...
            <div className="text-center">
                <p className="text-gray-400 flex items-center gap-2">
                    <RecordIcon/> Use the main controls to start a new recording.
                </p>
            </div>
            ...
            <button onClick={() => fileInputRef.current.click()} className="flex items-center gap-3 px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-lg hover:bg-blue-500 transition-colors">
                <UploadIcon /> Upload a Video File
            </button>
    ...
    const renderContent = () => {
        ...
        return (
            <div className="w-full h-full relative bg-gray-900">
                <div className={`absolute inset-0 w-full h-full flex items-center justify-center transition-opacity duration-300 ${editorMode === 'recording' ? 'opacity-100 z-10' : 'opacity-0 -z-10'}`}>
                    <video ref={videoRef} className="block w-full h-full object-contain -scale-x-100" playsInline muted autoPlay />
                </div>
                <div className={`absolute inset-0 w-full h-full transition-opacity duration-300 ${editorMode === 'welcome' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    {renderWelcomeScreen()}
                </div>
            </div>
        );
